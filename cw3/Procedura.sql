...
UPDATE Student SET IdEnrollment = @IdEnrollment WHERE IdEnrollment =  (SELECT IdEnrollment FROM Enrollment JOIN Studies ON Enrollment.IdStudy = Studies.IdStudy WHERE Studies.Name = @Studies AND Enrollment.Semester = @Semester);
END